...
import { Search, BrainCircuit, Database, History } from 'lucide-react';

interface LoadingScreenProps {
  stage: 'searching' | 'analyzing' | 'backtesting';
}
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-8">
      <div className="relative mb-8">
        ...
        <div className="relative z-10 w-24 h-24 bg-slate-800 rounded-full flex items-center justify-center border border-slate-700 shadow-2xl">
          {stage === 'searching' ? (
            ...
            <History className="text-emerald-400 animate-spin-slow" size={40} />
          )}
        </div>
      </div>

      <h3 className="text-2xl font-bold text-white mb-2">
        {stage === 'searching' ? 'Scouting Matches...' : stage === 'analyzing' ? 'Running Simulation Models...' : 'Running Historical Backtest...'}
      </h3>
      <p className="text-slate-400 max-w-md mb-8">
        {stage === 'searching' 
          ...
          : 'Simulating strategy against historical data to verify compounding mechanics and risk limits.'}
      </p>
      ...
      <div className="flex items-center gap-4 mt-8">
        ...
        <div className={`flex items-center gap-2 ${stage === 'analyzing' ? 'text-purple-400' : stage === 'backtesting' ? 'text-emerald-400' : 'text-slate-600'}`}>
            {stage === 'backtesting' ? <History size={16} /> : <BrainCircuit size={16} />}
            <span className="text-sm font-medium">{stage === 'backtesting' ? 'Historical Simulation' : 'AI Analysis'}</span>
        </div>
      </div>
    </div>
  );
